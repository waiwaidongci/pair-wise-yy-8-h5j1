...
import { buildVatBoard, ensureVatRegistry } from "./lib/vat-occupancy.js";
import { applyVatChange, recentHandovers, VatChangeError } from "./lib/vat-changes.js";
...
async function loadDb() {
  ...
  return ensureVatRegistry(JSON.parse(await readFile(dbPath, "utf8")));
}
...
<body>
  <header><div><h1>古法纸浆发酵记录</h1><div class="meta">纸浆批次、浸泡缸、换缸交接和异常观察</div></div><button id="reload">刷新</button></header>
  <main>
    <section>
    ...
    <section>
      <div class="panel" id="vatBoard" style="margin-bottom:14px"></div>
  ...
  <script>
    ...
    function render() {
      ...
      VatUI.enhanceCards(cards, load);
    }
    function fmtAt(at) { return at ? String(at).slice(5,16).replace('T',' ') + ' · ' : ''; }
    function cardHtml(item) {
      const main = fields.slice(0,4).map(([key,label]) => '<div><b>'+label+'</b> '+(item[key] ?? '')+'</div>').join('');
      const tasks = (item.tasks || []).map(t => '<div class="meta">任务 '+t.position+' · '+t.status+' · '+t.tension+'</div>').join('');
      const logs = (item.logs || []).slice().sort((a,b) => String(a.at).localeCompare(String(b.at))).slice(-6).map(l => '<div>'+fmtAt(l.at)+l.step+(l.vat ? ' <span class="pill">'+l.vat+'</span>' : '')+'：'+(l.note || '')+'</div>').join('');
      const changeForm = '<form data-vat-change="'+(item.id || item.code)+'" data-current-vat="'+(item.vat || '')+'"><label>换缸交接 · 当前缸 '+(item.vat || '未入缸')+'</label><select name="toVat" required></select><input name="at" type="datetime-local" title="交接时间"><input name="handler" placeholder="经手人" required><input name="temperature" placeholder="交接水温℃" required><button type="submit">保存换缸</button></form>';
      return '<article class="card"><h3>'+(item.code || item.id)+'</h3><span class="pill">'+item.status+'</span>'+main+tasks+changeForm+'<label>状态</label><select data-status="'+(item.id || item.code)+'">'+stages.map(s => '<option '+(s===item.status?'selected':'')+'>'+s+'</option>').join('')+'</select><button class="secondary" data-note="'+(item.id || item.code)+'">追加备注</button><div class="logs meta">'+(logs || '暂无记录')+'</div></article>';
    }
    async function load() { items = await api('/api/items'); await VatUI.mountBoard(document.querySelector('#vatBoard')); render(); }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/vat-ui.js") {
      res.writeHead(200, { "Content-Type": "text/javascript; charset=utf-8" });
      return res.end(await readFile(join(__dirname, "public", "vat-ui.js"), "utf8"));
    }
    if (req.method === "GET" && url.pathname === "/api/items") return send(res, 200, db.items.map(summarize));
    if (req.method === "GET" && url.pathname === "/api/vats") {
      const board = buildVatBoard(db).map(vat => ({ ...vat, handovers: recentHandovers(db, vat.name) }));
      return send(res, 200, { vats: board });
    }
    ...
    if (patch && req.method === "PATCH") {
      ...
      const input = await body(req);
      // 缸号与历史记录不允许直接改，换缸必须走交接单
      for (const key of ["vat", "logs", "observations", "vatChanges"]) delete input[key];
    ...
    if (action && req.method === "POST") {
      ...
      item.observations ||= [];
      // 观察保留当时缸位，换缸后仍能顺时间线回看
      item.observations.push({ at: new Date().toISOString(), vat: item.vat || "", ...input, abnormal });
    ...
    if (vatChange && req.method === "POST") {
      ...
      try {
        ...
        await saveDb(db);
        return send(res, 201, { item, change: record });
      } catch (error) {
        if (error instanceof VatChangeError) return send(res, error.status, { error: error.code, message: error.message });
        throw error;
      }
    }
